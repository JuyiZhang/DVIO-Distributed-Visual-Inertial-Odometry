    ...
    void UDPListenForData()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < dataLength; i++)
            {
                ...
                {
                    int coordinateID = BitConverter.ToInt32(data, i * 20 + 8);
                    Vector3 coordinatePos = BytesToVector3(data, i * 20 + 12);
                    float rotateY = BitConverter.ToSingle(data, i * 20 + 24);
                    if (observedHumanPosition.ContainsKey(coordinateID))
                        observedHumanPosition[coordinateID] = new DevicePosition(coordinatePos, rotateY);
                    else
                        observedHumanPosition.Add(coordinateID, new DevicePosition(coordinatePos, rotateY));
                    Debug.Log("Add coordinate: " + coordinatePos.ToString());
                }
            }
            float timestamp = 0;
            ...
            {
                timestamp = BitConverter.ToSingle(data, dataLength * 20 + 8);
                float timenow = (float)(DateTime.Now - epochStart).TotalMilliseconds;
                Debug.Log("Received observed coordinate with time" + (timenow - timestamp).ToString());
                coordinateObservationReceived?.Invoke();
            }

            Debug.Log("Received updated device position as " + devicePositions.ToString());
        }
    }
